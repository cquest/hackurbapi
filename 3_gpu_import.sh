...
psql gpu -c "
create view gpu_prescription_json as select json_build_object('type','Feature','properties',json_build_object('layer','prescription','libelle',libelle,'txt',txt,'typepsc',typepsc,'nomfic',nomfic,'urlfic',urlfic,'insee',insee,'datappro',datappro,'datvalid',datvalid,'id',id,'idurba',idurba,'libeattr1',libeattr1,'libeattr2',libeattr2,'libeattr3',libeattr3,'memo',memo),'geometry',st_asgeojson(wkb_geometry)::json)::text as geojson ,*, 'prescription'::text as layer from gpu_prescription ;

create view gpu_zone_urba_json as select json_build_object('type','Feature','properties',json_build_object('layer','zone_urba','idurba',idurba,'libelle',libelle,'libelon',libelong,'typezone',typezone,'destdomi',destdomi,'nomfic',nomfic,'urlfic',urlfic,'insee',insee,'datappro',datappro,'datvalid',datvalid,'id_zone',id_zone,'nom_com',nom_com,'typedoc',typedoc,'typezo_txt',typezo_txt,'destdo_ixi',destdo_txt,'producteur',producteur,'nomref',nomref,'dateref',dateref,'memo',memo,'fictif',fictif),'geometry',st_asgeojson(wkb_geometry)::json)::text as geojson ,*,'zone_urba'::text as layer from gpu_zone_urba ;

create view gpu_info_json as select json_build_object('type','Feature','properties',json_build_object('layer','info','libelle',libelle,'txt',txt,'typeinf',typeinf,'nomfic',nomfic,'urlfic',urlfic,'insee',insee),'geometry',st_asgeojson(wkb_geometry)::json)::text as geojson ,*, 'info'::text as layer from gpu_info ;

create view gpu_secteur_cc_json as select json_build_object('type','Feature','properties',json_build_object('layer','secteur_cc','idurba',idurba,'libelle',libelle,'typesect',typesect,'fermreco',fermreco,'destdomi',destdomi,'nomfic',nomfic,'urlfic',urlfic,'insee',insee,'datappro',datappro,'datvalid',datvalid,'libelong',libelong),'geometry',st_asgeojson(wkb_geometry)::json)::text as geojson ,*,'secteur_cc'::text as layer from gpu_secteur_cc ;
